...
| what declares the handler | a tag under `when@demo` | `#[AsNexusServiceHandler]` | n/a | six lines of `config/durable.php` |
| profile that **serves** | `APP_ENV=demo` (DBAL journal, dashboard unchanged) | `APP_ENV=dev` (Temporal journal) | n/a | `temporal` backend |
...
| `magento-workflows` | `magento/` | n/a | advances `OrderNexusWorkflow` |
| `logistics-serves-delivery` | `laravel/` | n/a | polls the Nexus tasks of `demo-laravel` |
| `logistics-workflows` | `laravel/` | n/a | advances `ShipWorkflow` |
