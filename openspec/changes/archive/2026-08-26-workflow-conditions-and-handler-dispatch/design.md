...
The consequence for the domain is that update dispatch cannot be fed from the history source
alone: the worker must hand the pending protocol messages to the execution for the first pass.
Signals need nothing of the sort.
