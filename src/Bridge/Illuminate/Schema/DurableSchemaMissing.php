...
final class DurableSchemaMissing extends \RuntimeException implements ExceptionInterface
{
    ...
    public static function insideTransaction(array $tables): self
    {
        return new self(\sprintf(
            'Durable\'s tables %s are missing, and a transaction is open: creating them here would commit it implicitly on MySQL. Run "php artisan migrate" before the first write.',
            implode(', ', $tables),
        ));
    }
}
