...
final class Configuration implements ConfigurationInterface
{
    public function getConfigTreeBuilder(): TreeBuilder
    {
        ...
        $treeBuilder->getRootNode()
            ...
            ->info('Nom de la connexion Doctrine DBAL à utiliser pour les stores Durable en mode dbal (event_store, workflow_metadata, activity_transport dbal, parent_link_store). Connexion distincte de celle du reste de l’app si vous déclarez p.ex. une connexion `durable` dans doctrine.dbal.connections.')
            ->end()
            ->arrayNode('event_store')
            ->addDefaultsIfNotSet()
            ->children()
            ->enumNode('type')->values(['in_memory', 'dbal', 'temporal'])->defaultValue('in_memory')->end()
            ...
            ->scalarNode('dsn')
            ->defaultNull()
            ->info('DSN temporal://… (ex. %env(DURABLE_TEMPORAL_DSN)%) — obligatoire si event_store.type = temporal. Requiert ext-grpc.')
            ->end()
            ->end()
            ->end()
            ->end()
