...
final readonly class NexusService
{
    ...
    {
        if ('' === $name) {
            throw new \InvalidArgumentException('A Nexus service name is not set.');
        }
        if ('' === trim($name)) {
            throw new \InvalidArgumentException('A Nexus service name cannot be blank: the server would accept it and no handler would ever match it.');
        }
        if ($name !== trim($name)) {
            throw new \InvalidArgumentException(\sprintf(
                'A Nexus service name "%s" has leading or trailing whitespace. The server records it as given, so a handler registered under the trimmed name would never be matched.',
                $name,
            ));
        }
        if (1 === preg_match('/[\x00-\x1F\x7F]/', $name)) {
            throw new \InvalidArgumentException(\sprintf(
                'A Nexus service name "%s" contains a control character; such names are invisible in logs and impossible to match by eye.',
