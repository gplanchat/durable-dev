...
final class EventSerializer
{
    ...
    public static function deserialize(array $row): Event
    {
        ...
        if (!\is_string($eventType)) {
            throw new \InvalidArgumentException('deserialize: missing event_type');
        }
        $executionId = $row['execution_id'];
        if (!\is_string($executionId)) {
            throw new \InvalidArgumentException('deserialize: missing execution_id');
        }
        $rawPayload = $row['payload'] ?? null;
        $payload = \is_string($rawPayload) ? json_decode($rawPayload, true, 512, \JSON_THROW_ON_ERROR) : $rawPayload;
        if (!\is_array($payload)) {
            throw new \InvalidArgumentException('deserialize: payload must be array or JSON object');
        }
        ...
        return match ($eventType) {
            ...
            ChildWorkflowScheduled::class => self::deserializeChildWorkflowScheduled($executionId, $payload),
            ChildWorkflowCompleted::class => new ChildWorkflowCompleted($executionId, (string) $payload['childExecutionId'], $payload['result'] ?? null),
            ChildWorkflowFailed::class => self::deserializeChildWorkflowFailed($executionId, $payload),
            ...
            WorkflowCancellationRequested::class => self::deserializeWorkflowCancellationRequested($executionId, $payload),
     ...
     */
    private static function deserializeChildWorkflowScheduled(string $executionId, array $p): ChildWorkflowScheduled
    {
     ...
     * @param array<string, mixed> $p
     */
    private static function deserializeChildWorkflowFailed(string $executionId, array $p): ChildWorkflowFailed
    {
     ...
     * @param array<string, mixed> $p
     */
    private static function deserializeWorkflowCancellationRequested(string $executionId, array $p): WorkflowCancellationRequested
    {
