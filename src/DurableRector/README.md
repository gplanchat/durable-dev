...
one iterable on that side and variadic on this one, and `Promise::some($p, 2)` becomes `some(2, ...$p)`.
...
`awaitWithTimeout($t, $c)` becoming `await($c, $t)`, and more than one does not, because rewriting
...
`\Generator` loses it too, since otherwise the class would widen its own contract, which is fatal.
