...
final class ExecutionContext
{
    ...
    public function activity(string $name, array $payload = [], ?ActivityOptions $options = null): Awaitable
    {
        ...
        $activityId = $scheduled ?? (string) Uuid::v7();
        $deferred = new \Gplanchat\Durable\Awaitable\Deferred();
     ...
     */
    public function timer(float $seconds): Awaitable
    {
     ...
     */
    public function continueAsNew(string $workflowType, array $payload = []): never
    {
        throw new ContinueAsNewRequested($workflowType, $payload);
    }
    ...
    public function executeChildWorkflow(string $childWorkflowType, array $input = [], ?ChildWorkflowOptions $options = null): Awaitable
    {
        ...
        if (null === $scheduledId && null !== $options->workflowId) {
            $this->assertChildExecutionIdUnused($childExecutionId);
        }
    ...
    public function delay(float $seconds): Awaitable
    {
        ...
        if (null === $scheduled) {
            ...
            $this->eventStore->append(new TimerScheduled($this->executionId, $timerId, $scheduledAt));
        }
    ...
    private function assertChildExecutionIdUnused(string $childExecutionId): void
    {
        foreach ($this->eventStore->readStream($childExecutionId) as $_) {
