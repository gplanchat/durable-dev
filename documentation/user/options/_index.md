...
A negative duration is rejected, and so is a computed `INF` or `NAN`, since an infinite duration is
...
RetryLimit::ofRetries(2);       // two retries, so three attempts
...
a name nobody polls, and the execution waits, with nothing in the logs.

> [!NOTE]
> Neither object catches a typo that is still a valid name, such as `payments-activites` for
...
A recurrence, validated at construction, since a typo would otherwise only appear when the server
