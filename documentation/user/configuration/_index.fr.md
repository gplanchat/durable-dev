...
| `lock_ttl` | flottant, secondes | `300` | Combien de temps un verrou de reprise survit à un worker mort en le tenant. Il n'est pas rafraîchi pendant une passe, donc **il doit dépasser la plus longue passe de reprise** : au-delà, un second worker rejoue la même exécution en parallèle. Une passe rejoue le journal et planifie les commandes suivantes ; les activités tournent en dehors, si bien qu'une passe dure en général bien moins d'une seconde. Seul un message reçu prend le verrou : envoyer une reprise n'attend jamais un worker. |
