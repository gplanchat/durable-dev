...
|---|---|---|---|
| `workflow-replay-divergence-guard` | **20 / 20** | Landed, on a design settled by [DUR042](../adr/DUR042-replay-divergence-guard.md) | nothing (it was `workflow-versioning`'s blocker) |
| `query-plumbing-leaves-the-environment` | 23 / 24 | Wiring, finished; its last task is blocked on tests it did not write | nothing |
| `workflow-versioning` | 0 / 18 | A probe, then wiring | The Rector set (§6), the comparison page's honesty (§4) |
| `nexus-handler-side` | 28 / 31 | Built. The probe reported, and the build followed it | nothing |
...
| API Platform state processor | Wiring: one class, two adapters | nothing |
...
| WooCommerce, Drupal, PrestaShop 9, Ibexa | not assessed | Not now (OST003 §6) |
...
| Saga helper | Wiring, small | nothing |
| ~~Rector bucket 1~~ | Wiring | **Done** |
| API Platform state processor | Wiring | nothing |
| Shopware 6, Sulu | Wiring | A real user |
| ~~Rector bucket 3~~ | Wiring | **Done** |
| `workflow-versioning` | A probe, then wiring | nothing (the guard has landed; the probe is taken) |
